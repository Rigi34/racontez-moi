        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="space-y-6">
            {posts.map((post) => (
              <article
                key={post.slug}
                className="bg-blanc p-8 shadow-[5px_5px_0px_#DAD4C5]"
              >
                <h2 className="font-display font-normal text-2xl mb-3 leading-[1.3]">
                  <Link
                    href={`/blog/${post.slug}`}
                    className="text-petrole hover:text-encre transition-colors"
                  >
                    {post.title}
                  </Link>
                </h2>
                <p className="font-serif text-base leading-[1.6] text-grege mb-4">
                  {post.description}
                </p>
                <Link
                  href={`/blog/${post.slug}`}
                  className="inline-block bg-encre text-blanc rounded-full font-sans text-sm px-5 py-2 hover:bg-[#3A3632] transition-colors"
                >
                  Lire l&apos;article →
                </Link>
              </article>
