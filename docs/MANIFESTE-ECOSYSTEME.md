...
supabase/migrations/   20 migrations SQL séquentielles, documentées
...
20 migrations SQL séquentielles. Tables clés : `sessions`, `fragments` (+ `fragments_historique`), `tours_conversation`, `profil_narrateur`, `banque_questions`, `livres_reference` (RAG), `abonnements`, `codes_cadeau`, `commandes_livre`, `adresses_livraison`, `photos`, et depuis le 21/08/2026 `usage_api` (migration `0019`) et `usage_anonyme` (migration `0020`) pour le rate limiting (§6.2).
