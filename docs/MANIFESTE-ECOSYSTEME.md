...
| Observabilité | Sentry (`@sentry/nextjs`) | Error tracking serveur/edge/client — depuis le 29/08/2026 (§6.2, A12) |
...
| A11 (🟠 haute) | Couverture de test étendue au-delà du rate limiting — `lib/redaction.ts`, `lib/codes-cadeau.ts`, idempotence webhook Stripe, `lib/typst.ts` | **Fait** — 26/08/2026 : `lib/typst.test.ts` (gouttière, échappement Typst, typographie française, compilation réelle via `NodeCompiler` sur un document multi-fragments avec gouttière non nulle), `lib/codes-cadeau.test.ts` (génération, idempotence webhook rejoué, retry sur collision, abandon après 5 tentatives, erreur inattendue non retentée) et `lib/redaction.test.ts` (formatage des tours, instruction de régénération ciblée, troncature `max_tokens` journalisée) — tous avec de vrais mocks Anthropic/Supabase, pas de comportement inventé. 01/09/2026 : `app/api/stripe/webhook/route.test.ts` ajouté (6 tests) — signature invalide, paiement non confirmé, parcours cadeau (création + non-recréation si événement rejoué), parcours abonnement (upsert scopé `user_id`, rejeu produisant le même état). Dernier point de couverture identifié pour A11, désormais clos |
| A12 (🟡🟠 moyenne-haute) | Error tracking structuré (Sentry ou équivalent) | **Fait** — 29/08/2026. SDK `@sentry/nextjs`, capture serveur/edge/client via `instrumentation.ts`/`instrumentation-client.ts`, `next.config.ts` enveloppé via `withSentryConfig`. Vérifié en réel : erreur de test déclenchée en dev, remontée confirmée dans le dashboard Sentry (issue groupée, stack trace exacte, release liée au commit `9045ad2`). DSN dans `.env.local` (non commité) — **reste à ajouter manuellement dans les variables d'environnement Vercel** pour que ça fonctionne en production |
...
| 26/08/2026 | A9 passé à **Fait** — audit RLS complet des 21 migrations. Trou réel trouvé sur `commandes_livre` (policy SELECT seule, alors que le code y écrit via le client RLS) : commande réelle de livre probablement cassée en production. Migration `0021` écrite pour corriger. A10 passé à **Fait** — 6 usages `service role` du dépôt revus, tous correctement scopés, aucune correction nécessaire |
| 26/08/2026 | Personnalisation du livre (titre/sous-titre/couleur) implémentée — migration `0022` (§6.1) |
| 30/08/2026 | Migrations `0021` et `0022` **appliquées à la base Supabase de production** (projet `fasvqpokgdvzahqmjlxz`, identifié via les variables d'environnement Vercel après confusion initiale avec un autre projet vide connecté par erreur). `0021` a nécessité un deuxième essai suite à un deadlock transitoire (collision avec du trafic réel), sans effet de bord — vérifié via `pg_policies` avant/après. La commande de livre imprimé et la personnalisation du livre sont désormais fonctionnelles en production, plus seulement dans le code |
