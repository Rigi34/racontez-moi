...
| A10 (🟡 moyenne) | Revue exhaustive des usages `service role` (contournement RLS volontaire) sur l'ensemble du dépôt | **Non fait** — 3 usages déjà confirmés correctement scopés (webhook Stripe, suppression de compte, activation cadeau) ; l'audit A9 du 26/08 a repéré 3 usages supplémentaires non encore passés en revue : `app/api/cadeau/certificat/[code]/image/route.ts`, `app/api/cadeau/certificat/[code]/pdf/route.ts`, `app/offrir/merci/page.tsx` |
